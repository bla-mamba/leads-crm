...
Go to: https://supabase.com/dashboard/project/kwiuzntxxsmezjgswact/sql/new
...
DROP POLICY IF EXISTS "Users can update their own notifications" ON lead_notifications;
DROP POLICY IF EXISTS "System can create notifications" ON lead_notifications;
...
SELECT 'Notification system installed successfully!' as message;
SELECT count(*) as notification_count FROM lead_notifications;
```

### Step 3: Enable Realtime (IMPORTANT!)

After running the SQL, you MUST enable Realtime for the notifications to work:
...
4. This allows real-time notifications to be pushed to users

### Step 4: Test the System

1. Refresh your application
2. Login to your CRM
3. Look for the bell icon in the sidebar (top right area)
4. Create a new lead (if you're an admin)
5. You should immediately:
   - Hear a notification sound
   - See the bell icon show a red badge with "1"
   - See a toast notification
   - See a browser notification (if you granted permission)

## What This Migration Does

### Creates Table
- `lead_notifications` - Stores all notification records

### Creates Indexes
- Optimizes queries for user ID, read status, and timestamps
...
### Creates Functions
1. `create_lead_notifications()` - Evaluates hierarchy and creates appropriate notifications
2. `trigger_lead_notifications()` - Fires when new leads are created
3. `trigger_assignment_notifications()` - Fires when leads are assigned

### Creates Triggers
1. `on_lead_insert_notification` - Automatic notification on new lead
2. `on_lead_assignment_notification` - Automatic notification on assignment

## Notification Rules

### Admin Users
- Get notified about **ALL** new leads

### Desk Users
- Get notified about leads in their desk
- Get notified about leads assigned to their team members

### Manager Users
- Get notified about leads assigned to them
- Get notified about leads assigned to their agents

### Agent Users
- Get notified **ONLY** about leads assigned directly to them
...
- **Enable Realtime replication** for the `lead_notifications` table
- Check browser console for subscription errors
- Refresh the page after enabling Realtime

### No Sound
- Grant browser notification permission
- Check that `/public/notification.mp3` exists
- Interact with page before testing (browsers block audio on page load)
...
- `src/components/Sidebar.tsx` - Shows notification bell
